...
basic_1_test() ->
    ...
    Expected = {ok,{relp, 5253, syslog, 222, <<"<6>2010-11-14T20:53:46.473766+00:00 obelisk kernel: [965250.511753] iptables(INPUT-DROP): IN=br0 OUT= MAC= SRC=192.168.1.69 DST=192.168.1.255 LEN=156 TOS=0x00 PREC=0x00 TTL=64 ID=0 DF PROTO=UDP SPT=17500 DPT=17500 LEN=136 ">>},<<>>},
...
basic_2_test() ->
    ...
    Expected = {ok,{relp, 5580, syslog, 91, <<"<83>2010-11-14T21:40:01.993837+00:00 obelisk sudo: pam_unix(sudo:auth): conversation failed">>},<<>>},
    Returned = relp_protocol:decode(Input),
...
basic_3_test() ->
    ...
    Expected = {ok,{relp, 1, open, 85, <<"relp_version=0\nrelp_software=librelp,1.0.0,http://librelp.adiscon.com\ncommands=syslog">>},<<>>},
    Returned = relp_protocol:decode(Input),
...
basic_5_test() ->
    ...
    Expected = {ok,{relp, 28834, rsp, 6, <<"200 OK">>},<<>>},
    Returned = relp_protocol:decode(Input),
...
missing_1_test() ->
    Test = "missing_1",
    Input = <<"76 syslog 221 <6>2010-11-14T06:48:16.925258+0">>,
    Expected = {more,221},
    Returned = relp_protocol:decode(Input),
